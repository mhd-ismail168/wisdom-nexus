...
export const ELECTRIC_CONFIG = {
	...
	svg: {
		strokes: {
			outer: { width: 3, color: 'rgba(184,156,74,0.6)' }, // wide soft gold halo
			mid: { width: 2.2, color: 'rgba(200,175,80,0.45)' }, // warm gold glow
...
export function Component({
	...
	// Resolve what to render for each side
	const resolvedLeft = leftComponent ?? (
		leftImage ? (
			<Image src={leftImage} alt={leftAlt} fill className="object-cover" />
		) : (
			<div className="flex min-h-screen min-w-screen h-full w-full items-center justify-center bg-black">
				<h1 className="text-6xl font-bold text-neutral-300">A</h1>
			</div>
		)
	)

	const resolvedRight = rightComponent ?? (
		rightImage ? (
			<Image src={rightImage} alt={rightAlt} fill className="object-cover" />
		) : (
			<div className="flex min-h-screen min-w-screen h-full w-full items-center justify-center bg-neutral-950">
				<h1 className="text-6xl font-bold text-neutral-400">B</h1>
			</div>
		)
	)
